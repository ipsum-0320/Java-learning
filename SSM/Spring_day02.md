...
![image-20240708162753876](./assets/image-20240708162753876.png)

![image-20240708164603613](./assets/image-20240708164603613.png)

### 1.2.DataSource（数据库连接池）

数据库连接池就是一个容器。

![image-20240708164840884](./assets/image-20240708164840884.png)

![image-20240708165041342](./assets/image-20240708165041342.png)
...
![image-20240708172903666](./assets/image-20240708172903666.png)
...
在配置类上添加`@Configuration`注解，将其标识为一个配置类,替换`applicationContext.xml`，这个注解会替换掉：

![image-20240708195212142](./assets/image-20240708195212142.png)
...
> `@Autowired` 默认按照类型注入，当 bean 有多个时，会按照名称注入。
...
对于被 `@Bean` 修饰的方法，如果需要接受 Spring 容器中的 Bean，**那么需要在方法中指定相应类型的参数**。
...
#### 6.3.3.最新版的 Spring 整合 JUnit

代码如下所示，添加了 `@SpringBootTest` 和 `@Test` 注解：
...
作用和 @Autowired 一样，用来进行自动装配，但是 @Resource 是默认 byName 的，而 @Autowired 是默认 byType 的。

关于 Bean 的 name，当Spring扫描到一个使用@Component进行标注的类时，默认会定义一个beanName为我们的类名的bean，对应首字母小写。所以在下面示例中默认就会生成一个类型为Hello，beanName为“hello”的bean。当然我们也可以通过@Component的value属性来指定需要生成的bean的beanName。如下示例中就通过value属性指定了对应生成的bean的beanName为“abc”。
