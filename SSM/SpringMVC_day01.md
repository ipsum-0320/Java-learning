...
因为 SpringMVC 是一个 Web 框架，将来是要替换Servlet,所以先来回顾下以前Servlet是如何进行开发的？
...
## 扩展加餐（非注解配置 SpringMVC）

ServletContainersInitConfig 是用于替换 web.xml 的，那么原来的 web.xml 应该如何配置呢？如下所示：
...
<web-app>
    <servlet>
        ...
        <init-param>
            ...
            <param-value>com.itranswarp.learnjava.SpringMvcConfig</param-value>
        </init-param>
...
初始化参数`contextClass`指定使用注解配置的`AnnotationConfigWebApplicationContext`，配置文件的位置参数`contextConfigLocation`指向`SpringMvcConfig`的完整类名，最后，把这个Servlet映射到`/*`，即处理所有URL。

上述配置可以看作一个样板配置，有了这个配置，Servlet容器会首先初始化Spring MVC的`DispatcherServlet`，在`DispatcherServlet`启动时，它根据配置`SpringMvcConfig`创建了一个类型是`WebApplicationContext`的IoC容器，完成所有Bean的初始化，并将容器绑到`ServletContext`上。
...
1. 服务器启动，执行ServletContainersInitConfig类（基于 SPI），初始化web容器
...
从包结构来看的话，Spring已经多把 SpringMVC 的 controller 类也给扫描到，所以针对这个问题该如何解决，就是咱们接下来要学习的内容。
...
> 在 Spring MVC 集成 Mybatis 的过程中，DAO 不需要显式地放到 IOC 容器中的原因是 Mybatis 提供了自动生成 DAO 实现类并将其注入到 IOC 容器中的机制。
>
> ![image-20240714210126388](./assets/image-20240714210126388.png)
...
**AbstractAnnotationConfigDispatcherServletInitializer 的发现使用了 SPI**。`SPI`，全称为 `Service Provider Interface`，是一种服务发现机制。它通过在`ClassPath`路径下的`META-INF/services`文件夹查找文件，自动加载文件里所定义的类。**也就是在服务启动的时候，Servlet会自动加载该文件定义的类**
...
@RequestMapping 可以加在类上面，也可以加在方法上面。
...
上面的方法是使用 SpringMVC 集成过滤器，而过滤器是 Servlet 的标准（也就是说原来是配置在 web.xml 中的），我们下面给出详细的在 SpringMVC 中集成过滤器的标准。

参考：https://www.liaoxuefeng.com/wiki/1252599548343744/1282384114745378。

![image-20240714220132210](./assets/image-20240714220132210.png)

如果使用 DelegatingFilterProxy 进行 Filter 的代理，那么流程如下：
...
此处又使用到了类型转换，内部是通过HttpMessageConverter 接口的实现类完成的，它可以实现:

* 对象转Json数据(POJO -> json)
* 集合转Json数据(Collection -> json)

![image-20240718200423728](./assets/image-20240718200423728.png)
...
查看REST风格的描述，你会发现请求地址变的简单了，并且光看请求URL并不是很能猜出来该 URL 的具体功能
...
* 区别
  * **@RequestParam用于接收url地址传参（也就是 query 传参）或表单传参**
...
**主要原因是 SpringMVC 拦截了所有的访问请求，但事实上静态资源的访问请求需要交给 Tomcat 处理。**
