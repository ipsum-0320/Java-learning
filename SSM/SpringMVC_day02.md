...
![image-20240718220647315](./assets/image-20240718220647315.png)
...
需要注意的是，SpringMVCConfig 和 SpringConfig 会创建两个容器对象，但是这两者并不一致，前者可以访问后者中的 Bean，而后者无法访问前者中的 Bean。这是一个父子容器的概念。

> **在 springboot 中，已经没有了父子容器，只有一个容器了，所以 Service 可以注入 Controller 方法**。
...
注意最后还需要添加事务管理，需要额外配置一个 Bean：

![image-20240719201203575](./assets/image-20240719201203575.png)

## 2，统一结果封装

这和 kwai 的封装是一致的：

![image-20240719201728889](./assets/image-20240719201728889.png)
...
注意：**异常处理器的返回值就是 Controller 返回给前端的值。**
...
> * Exception：定义方法时必须声明所有可能会抛出的exception； 在调用这个方法时，必须捕获它的checked exception，不然就得把它的exception传递下去；exception是从java.lang.Exception类衍生出来的。例如：IOException，SQLException就属于Exception。

关系图如下：

![image-20240719210421295](./assets/image-20240719210421295.png)
...
* 方式一:`try{}catch(){}`在catch中重新throw我们自定义异常即可（后续可将可预见的系统异常全部交给 AOP 处理）。
* 方式二:直接throw自定义异常即可
...
> 因为 ServletConfig 中拦截了所有的请求，这个配置会把所有的请求转发给 Dispatcher。
...
> handler 参数可以通过反射机制拿到 Controller 层的方法，例如上面举例的 `BookController.save()`。
