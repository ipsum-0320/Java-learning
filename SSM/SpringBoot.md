...
> Jar 包其实就是 `.class` 文件的压缩包。==注意：该插件必须配置，不然打好的 `jar` 包也是有问题的。==
...
解释一下这里的 classpath 和 file：

* classpath，如下：

  ![image-20240725173913981](./assets/image-20240725173913981.png)

* file：打出来的 jar 包所在的目录。
...
除了上面的配置外，如果我们使用 `*.xml` 来编写 mybatis 的 curd 配置文件，那么还需要定义两个额外的配置：

```yml
mybatis:
	type-aliases-package: com.demo.domain # POJO 类所在的包，简化 resultType 的编写。
	mapper-locations: classpath:/mybatis/*.xml # mapper scan 相关的配置，用于写 SQL 的 *.xml 文件。
```
...
写在 Dao 层的接口上可以生成相应的代理类。

项目中最好是分别在启动类和 Mappr 接口加上@MapperScan注解和@Repository注解的搭配使用，这样将会自动扫描所有mapper接口生成实现类，不需要为每个mapper接口单独加@Mapper注解，还能够消除“注入”的错误提示的作用。
...
4. 单独使用@Mapper时，可能会在编辑器出现警告，不影响程序运行。可以配合使用@Repository消除警告。

@Mapper 可以视作 Mybatis 原来的 MapperScan 属性。

> 注意，@Mapper 可以和注解使用，也可以和 xml 文件使用，只要 xml 中的 mapper.namespace 对齐 DAO 的文件路径就好。

#### lombok 中常见注解的作用

**@Data 的作用**，在类上添加 @Data 注解后，Lombok 会自动生成以下方法：

- 所有字段的 getter 和 setter 方法。
- equals 和 hashCode 方法，用于比较两个对象是否相等。
- toString 方法，用于将对象转换为字符串表示形式。

**@NoArgsConstructor | @AllArgsConstructor 的作用**， 用在类上，自动生成无参构造和使用所有参数的构造函数。
