...
- type：字段数据类型，常见的简单类型有：
  ...
  - 日期：date
  - 对象：object
- **index：是否创建索引，默认为true（根据字段设置，也就是说其是根据字段绑定的属性）**
- analyzer：使用哪种分词器
- properties：该字段的子字段



例如下面的json文档（**对比：一个文档就是一个数据库的 Row**）：
...
### 2.2.1.创建索引库（就相当于 MySQL 中的表）和映射
...
![image-20241012114113367](./分布式搜索引擎01.assets/image-20241012114113367.png)
...
我们学习的是Java HighLevel Rest Client 客户端 API
...
### 4.0.3.mapping映射分析（重点是是否参与搜索和是否分词）
...
创建了一个字段 all，该字段拥有 brand 字段的内容（因为 copy_to）。

这个功能主要是为了联合搜索（**同时根据多个字段去搜索**），例如即从 brand 的角度去查询，又从商圈的角度去查询，那么就可以使用 all 将 brand 和商圈的 text 聚合到一起。
