...
> 倒排索引是 term（词条，是分词的产物，或者直接就是整个 text）=> doc id 的映射，后续会根据 doc id 查询出整个 doc，然后返回所有字段 。

- **查询所有**：查询出所有数据（不加条件限制），一般测试用。例如：match_all。

- **全文检索（full text）查询**：利用分词器对用户输入内容分词，**然后去倒排索引库中匹配**。例如：
  - match_query
  - multi_match_query
- **精确查询**：根据精确词条值查找数据，一般是查找**keyword、数值、日期、boolean**等类型字段（不需要分词，但是也会建立倒排索引）。例如：
  - ids（根据 id 精确匹配）。
  - range。
  - term。
...
**search 和 get 是不一样的，search 会用到倒排索引，直接查一坨，get 则是精确查询某一个。**
...
FIELD 用来填充被搜索的字段是哪一个，TEXT 填写用户填充的内容，例如
...
**表示搜索出 all 字段中包括上海的所有的文档，同时 TEXT 也会被分词处理，匹配的词条越多，搜索排名越靠前**。
...
- match：根据一个字段查询
- multi_match：根据多个字段查询，参与查询字段越多，查询性能越差，使用 copy_to 进行查询。
...
每一个不同的字段，其查询的条件、方式都不一样，必须是多个不同的查询，而要**组合这些查询，就必须用bool查询了**。
...
> 一旦我们指定了 sort 排序算法，**那么就不再使用相关度算法进行排序了**。
...
> 这里的 FIELD **指定了要被排序的字段**。

排序条件是一个数组，也就是可以写多个排序条件。按照声明的顺序，当第一个条件相等时，再按照第二个条件排序，以此类推

**示例**：

需求描述：酒店数据**按照用户评价（score)降序排序，评价相同的按照价格(price)升序排序**。 
...
一个 search after 的例子是：

![image-20241015144026696](./分布式搜索引擎02.assets/image-20241015144026696.png)

这意味着从这个序号之后的数据，找 10 条出来。
...
因为 query 可能会查到多个文档，每个文档中都有 name，因此最后的高亮结果是一个数组，针对每个 name 都做**高亮**。
