...
用于反向代理的 NGINX 是静态资源 NGINX 服务器，而后面的 NGINX 集群则是业务 NGINX 集群。
...
1. 进行数据解耦，如果一张表的数据太多，**那么查询效率就会比较低**。
2. 另一方面我们会给数据加缓存，如果所有数据项都在一张表中，那么将来一旦有一项数据发生改变，那么整条数据项的缓存都将失效。**从而使得缓存经常未命中**。

nginx 配置如下所示：

![image-20240125222516704](多级缓存.assets/image-20240125222516704.png)
...
- **分布式缓存（一般是指 Tomcat 是集群状态）**，例如Redis：
...
void testBasicOps() {
    ...
    // 优先根据key查询JVM缓存，如果未命中，则执行参数二的Lambda表达式（查完数据之后会将其存储到内存中）
...
- **基于引用**：设置缓存为**软引用或弱引用**，利用GC来回收缓存数据。性能较差，不建议使用。
...
![image-20240125230045268](多级缓存.assets/image-20240125230045268.png)
...
首先，我们需要定义两个Caffeine的缓存对象，分别保存商品、库存的缓存数据（需要定义**配置类**）。
...
可以将 openresty 理解成加强版的 nginx（openresty 本身对 nginx 进行了增强）。Nginx 不再是一个简单的静态网页服务器，也不再是一个简单的反向代理了。openresty 致力于通过一系列 nginx 模块，把nginx扩展为全功能的 web 应用服务器。

openresty（Web 服务器）!== Tomcat，Tomcat 如下：

![image-20240126145114199](多级缓存.assets/image-20240126145114199.png)

**但是 openresty 确实既能返回静态资源，又能返回动态资源。**

> netty（可以自定义协议）是类似于 tomcat 一样的 Web 服务器。
...
而默认的负载均衡规则是**轮询模式**，当我们查询/item/10001时：
...
Spring 中的常用注解：https://juejin.cn/post/7067797042591973407

![image-20240126155630274](多级缓存.assets/image-20240126155630274.png)
...
现在，整个多级缓存中只差最后一环，也就是**nginx的本地缓存**了。如图：
...
**一些时效性要求较低，更新频率较低的数据才会放到 openResty 的本地缓存中，其余的一致性要求较高的数据（例如库存数据）会放到 Redis 缓存和 JVM 缓存中。openResty 的数据同步策略使用有效期同步，其余两个使用 Canal 异步通知策略。**
...
**有点类似于 redis 做持久化时的 AOF。**

而Canal就是把自己伪装成MySQL的一个slave节点，**从而监听master的binary log变化**。再把得到的变化信息通知给Canal的客户端，进而完成对其它数据库的同步。

> 因此要想实现 canal，就需要实现 mysql 的主从同步。
...
![image-20240126210721751](多级缓存.assets/image-20240126210721751.png)
