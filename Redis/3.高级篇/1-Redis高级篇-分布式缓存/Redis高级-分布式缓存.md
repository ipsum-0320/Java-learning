...
**bgsave开始时会fork主进程得到子进程**（显然，在 fork 过程中，主进程会被阻塞），**子进程共享主进程的内存数据（通过页表的方式共享）**。完成fork后读取内存数据并写入 RDB 文件。

fork采用的是copy-on-write技术：

- 当主进程执行读操作时，访问共享内存；
- 当主进程执行写操作时，则会拷贝一份数据（只拷贝被写的那份数据），执行写操作。
...
> 可以参考文章：https://zhuanlan.zhihu.com/p/340082703

![image-20210725151940515](assets/image-20210725151940515.png)

RDB 的 bgsave 会开启一个子进程。AOF 在往缓存区写时使用主进程，AOF 重写时使用子进程。

RDB 和 AOF 存在混合持久化方式，如下所示：

![image-20240124175921907](Redis高级-分布式缓存.assets/image-20240124175921907.png)
...
![image-20240124192943909](Redis高级-分布式缓存.assets/image-20240124192943909.png)

redis 本身就提供主从同步和读写分离，master 节点提供读功能和写功能，而 slave 节点只提供读功能。
...
> **这里的 repl_baklog 有一点像 AOF 在进行重写时的 AOF 缓存区。**
...
> replication id 声明同步哪一个 master 的数据，offset 记录同步的进度。

因为slave原本也是一个master，有自己的replid和offset，当第一次变成slave，与master建立连接时，发送的replid和offset是自己的replid和offset。

master判断发现slave发送来的replid与自己的不一致，说明这是一个全新的slave，就知道要做**全量同步**了。
...
- master将**RDB期间（包括 master 生成、传输和 slave 加载）**的命令记录在repl_baklog，并持续将log中的命令发送给slave
...
repl_baklog中会记录Redis处理过的命令日志及offset，包括**master当前的offset，和slave已经拷贝到的offset**：
...
但是，如果slave出现网络阻塞（也就是 **slave 节点宕机**），导致master的offset远远超过了slave的offset： 
...
- 在master中配置repl-diskless-sync yes启用无磁盘复制，避免全量同步时的磁盘IO。（**RDB 文件不写到磁盘中，而是直接写到网络 IO 流当中**）
- Redis单节点上的内存占用不要太大，减少RDB导致的过多磁盘IO（通过降低 Redis 存储数据量的方式减小 RDB 的体积）
...
- **自动故障恢复（实现主从切换）**：如果master故障，**Sentinel会将一个slave提升为master**。当故障实例恢复后也以新的master为主
- **通知**：**Sentinel充当Redis客户端的服务发现来源**，当集群发生故障转移（或者主从切换）时，会将最新信息推送给Redis的客户端，从而知道哪个 redis 是主节点，进行写操作。
...
- 首先会**判断 slave节点与 master节点断开时间长短**，如果超过指定值（down-after-milliseconds * 10）则会**排除**该 slave节点
...
- 最后，sentinel将故障节点标记为slave（配置文件添加 **SLAVEOF**），当故障节点恢复后会自动成为新的master的slave节点
...
如果接口内部只有一个方法，那么可以使用 lambda 表达式代替。

Lambda表达式简化匿名内部类的书写，但Lambda表达式并不能取代所有的匿名内部类，只能用来取代**函数接口（Functional Interface）**的简写。例如：
...
- **高并发写（因为只有一个 master redis 节点）**的问题
...
- **客户端请求可以访问集群任意节点，最终都会被路由转发到正确节点**（根据插槽路由）
...
> 数据和插槽绑定，插槽又和 redis 节点对应（这种对应关系是会改变的，redis 节点扩缩容都会导致对应关系的改变），综合来看，数据就和 redis 节点对应了。
...
> 对于 Redis 分片集群而言，其本身就具备了主从切换的故障恢复功能，而不需要哨兵。
...
- 缺省：默认的流程，如图1~6歩
- force：省略了对**offset（校验 slave 的 offset 和 master 的 offset 是否一致）**的一致性校验
...
> 自动具备读写分离（主从架构）、插槽自动路由（扩缩容）、故障转移恢复等功能。
