...
cookie、session 原理：参考文章 https://www.cnblogs.com/cswiki/p/17029477.html。
...
>ThreadLocalMap 的 Entry 中，key == ThreadLocal（弱引用），value == 线程局部变量缓存值（强引用）。

如果小伙伴们看过threadLocal的源码，你会发现在threadLocal中，无论是他的put方法和他的get方法， **都是先从获得当前用户的线程，然后从线程中取出线程的成员变量map**，只要线程不一样，map就不一样，所以可以通过这种方式来做到线程隔离。

![img](https://pic2.zhimg.com/v2-e2d4b8eac152596232d3e32313927d59_r.jpg)

一个线程可以有多个 ThreadlocalMap，ThreadlocalMap（本质其存储由 Entry 数组负责）的 key 是 Threadlocal，因此一个 Threadlocal 只能对应一个数据。

> https://juejin.cn/post/6932807532587810824 ThreadLocal 内存泄露的原因。
>
> ThreadLocalMap的`Entry`的Key是弱引用，如果外部没有强引用指向Key，Key就会被回收，而value由于`Entry`强引用指向了它，导致无法被回收，但是value又无法被访问，因此发生内存泄漏。 所以，开发者应该尽量在使用完毕后及时调用`remove()`删除节点。 如果开发者粗心忘记主动删除了，ThreadLocal也做了很多努力，在`get()`、`set()`、`remove()`时会自发的检查过期节点，并清理掉他们，最大程度的来避免程序发生「内存泄漏」，还是非常贴心的。

**threadlocal 是线程独享的数据**。
...
> 这是因为加载顺序导致的问题，`拦截器`加载的时间点在`springcontext`之前，而`Bean`又是由`spring`进行管理。因此 StringRedisTemplate 的注入需要放在 MvcConfig 中。
...
> @Resource 和 @Autowired 的异同：
>
> 1. 两者都可以写在字段和setter方法上。两者如果都写在字段上，那么就不需要再写setter方法。
> 2. @Autowired为Spring提供的注解，需要导入包org.springframework.beans.factory.annotation.Autowired;只按照byType注入。
...
> 4. @Resource默认按照ByName自动注入，由J2EE提供，需要导入包javax.annotation.Resource。@Resource有两个重要的属性：name和type，而Spring将@Resource注解的name属性解析为bean的名字，而type属性则解析为bean的类型。所以，如果使用name属性，则使用byName的自动注入策略，而使用type属性时则使用byType自动注入策略。如果既不制定name也不制定type属性，这时将通过反射机制使用byName自动注入策略。
> 5. **默认情况下，使用 @Bean声明一个bean，bean的名称由方法名决定**
...
需要注意的是，stringRedisTemplate 需要所有的 key value 都是 string，因此需要保证这一点，包括 hash 里面的 key value。

### 1.9 解决状态登录刷新问题

**拦截器和过滤器**的区别：https://blog.csdn.net/xinzhifu1/article/details/106356958/。

过滤器（Filter）：当你有一堆东西的时候，你只希望选择符合你要求的某一些东西。定义这些要求的工具，就是过滤器。 拦截器（Interceptor）：在一个流程正在进行的时候，你希望干预它的进展，甚至终止它进行，这是拦截器做的事情。
...
![image-20240117151217925](Redis实战篇.assets/image-20240117151217925.png)
...
`@Transactional`是**开启声明式事务**的方法。声明式事务管理建立在AOP之上的。其本质是对方法前后进行拦截，然后在目标方法开始之前创建或者加入一个事务，在执行完目标方法之后根据执行情况提交或者回滚事务。传播行为默认是`REQUIRED`。

传播行为指的是，如果多个事务进行嵌套运行，子事务是否要和大事务共用一个事务。主要有如下两个：
...
> 尽量避免 Boolean 返回，然后让 boolean 类型承接。
...
private boolean tryLock(String key) {
    ...
    // 为了防止自动拆箱的 NPE 错误，尽量不要将包装类（Boolean）赋值给原始类型（boolean）。
}
...
> 泛型的推断：一般来讲，Java 中的方法是能够做泛型的推断的，譬如如下方法声明：
>
> `<T> MyClass(T t)`，倘若没有对应泛型的参数，那么也可以直接传入类型，如下所示：`<R, ID> R queryWithLogicalExpire(String keyPrefix, ID id, Class<R> type)`。
...
> 全局 id 恰好和 long 类型的**位长度**一样，共有 64 位。
...
public class RedisIdWorker {
    ...
    public long nextId(String keyPrefix) {
        ...
        // 2.2.自增长
        long count = stringRedisTemplate.opsForValue().increment("icr:" + keyPrefix + ":" + date); // 防止 key 自增至溢出。
...
![image-20240118151226618](Redis实战篇.assets/image-20240118151226618.png)
...
假设线程1过来查询库存，判断出来库存大于 1，正准备去扣减库存，但是还没有来得及去扣减，此时线程 2 过来，线程 2 也去查询库存，发现这个数量一定也大于1，那么这两个线程都会去扣减库存，最终多个线程相当于一起去扣减库存，此时就会出现库存的超卖问题。
...
> 什么是自旋锁，自旋锁就是不进入阻塞状态，而是一直忙等待（占用CPU 资源，阻塞则不会占用 CPU 资源）。
>
> ![image-20240118164409785](Redis实战篇.assets/image-20240118164409785.png)
...
之前的方式要修改前后都保持一致，但是这样我们分析过，成功的概率太低，所以我们的乐观锁需要变一下，改成stock大于0 即可（**相当于做修改时会再查一次**）。
...
> 事务提交才会使得数据库修改，一般来讲事务要被锁包裹。
>
> 事务的开始由 Begin 开始。
>
> 事务的结束由 Commit 结束，Commit 之后修改就会提交至数据库；事务的结束也可以由 Rollback 结束，此时事务会失败，事务回滚。
...
Spring 事务失效的情况：https://juejin.cn/post/7179080622504149029

Spring 中事务的实现原理为——代理，所有就用一个事务注解`@Transactional`来控制事务，底层实现是基于切面编程`AOP`实现的，而`Spring`中实现`AOP`机制采用的是动态代理，具体分为`JDK`动态代理和`CGLIB`动态代理两种模式。
...
当发生了自身调用时，即该类调用了自己的方法，而没有经过 Spring 的代理类，默认只有在外部调用事务才会生效，这也是老生常谈的经典问题了。
...
![image-20240118204846555](Redis实战篇.assets/image-20240118204846555.png)
...
Mysql：mysql本身就带有锁机制，但是由于**mysql性能本身一般**，所以采用分布式锁的情况下，其实使用mysql作为分布式锁比较少见。
...
* 获取锁：
  ...
  * 互斥：确保只能有一个线程获取锁，将设置 nxkey 和设置过期时间定义为原子操作。
...
持有锁的线程在锁的内部出现了阻塞，导致他的锁自动释放，这时其他线程，线程2来尝试获得锁，就拿到了这把锁，然后线程2在持有锁执行过程中，线程1反应过来，继续执行，而线程1执行过程中，走到了删除锁逻辑，此时就会**把本应该属于线程2的锁进行删除**，这就是误删别人锁的情况说明。
...
需求：修改之前的分布式锁实现，满足：在获取锁时存入线程标示（可以用UUID标识不同的 JVM）。在释放锁时先获取锁中的线程标示，判断是否与当前线程标示一致
...
线程1现在持有锁之后，在执行业务逻辑过程中，他正准备删除锁，而且已经走到了条件判断的过程中，比如他已经拿到了当前这把锁确实是属于他自己的，正准备删除锁，但是此时他的锁到期了，那么此时线程2进来，但是线程1他会接着往后执行，当他卡顿结束后，他直接就会执行删除锁那行代码，相当于条件判断并没有起到作用，这就是删锁时的原子性问题，之所以有这个问题，是**因为线程1的拿锁，比锁，删锁，实际上并不是原子性的**，我们要防止刚才的情况发生，
...
* 利用**set nx ex获取锁**，并设置过期时间，保存线程标示
...
**主从一致性：** 如果Redis提供了主从集群，当我们向集群写数据时，主机需要异步的将数据同步给从机，而万一在同步过去之前，主机宕机了，就会出现**线程安全**问题。
...
public Result seckillVoucher(Long voucherId) {
    	...
    	// 这里的 getLock 的参数其实就是 redis 中的 key。
        //获取锁对象
...
>  以上就是可重入锁的原理，也就是先看锁是不是被本线程获取了，如果是，那么就计数加一。
>
>  voaltile 是什么？
>
...
![image-20240119110947094](Redis实战篇.assets/image-20240119110947094.png)

![image-20240119111003670](Redis实战篇.assets/image-20240119111003670.png)
...
![image-20240119191954418](Redis实战篇.assets/image-20240119191954418.png)

leaseTime == -1 会开启看门狗机制，利用其内置的释放时间（30s）。

watchDog 用来解决超时释放问题，即持有锁的时候会不断更新锁的有效期。

> 超时释放问题是指 =>
>
> 锁超时释放虽然可以避免死锁，但如果业务执行耗时比较长，也会导致锁释放，存在安全隐患。
...
接下来会有一个条件分支，因为lock方法有重载方法，一个是带参数，一个是不带参数，如果带带参数传入的值是-1，如果传入参数，则leaseTime（**释放时间**）是他本身，所以如果传入了参数，此时leaseTime != -1 则会进去抢锁，抢锁的逻辑就是之前说的那三个逻辑
...
![image-20240119192502802](Redis实战篇.assets/image-20240119192502802.png)
...
在这六步操作中，又有很多操作是要去**操作数据库**的，而且还是一个线程串行执行， 这样就会导致我们的程序执行的很慢，所以我们需要异步程序执行，那么如何加速呢？
...
* 基于Lua脚本，判断秒杀库存、一人一单，决定用户是否抢购成功
  ...
  > 这里需要弄明白，为什么秒杀不会出现超卖，以及一人一单时为什么不会出现多重下单，因为redis是单线程执行的，**不需要添加锁防止线程安全**。
...
![image-20240120153933678](Redis实战篇.assets/image-20240120153933678.png)
...
* 无法避免消息丢失（没有消息确认）。
* 只支持单消费者（被一个消费者拿走之后，别的消费者无法获得）。

### 7.3 Redis消息队列-基于PubSub的消息队列（阻塞式）
...
* **采用发布订阅模型，支持多生产、多消费**（当然也支持单消费，指定唯一消费者即可）。

缺点：

* 不支持数据持久化（消息发到队列中会直接**同步**转发，不做任何停留）。

  > 这里的持久化指代的是 redis 持久化，如 https://pdai.tech/md/db/nosql-redis/db-redis-x-rdb-aof.html 所示。 
* 无法避免消息丢失（没有消息确认）。
* 消息堆积有上限，超出时数据丢失。
...
> Stream 读完之后，消息并不会被删除，而是永久的存储，读取**最新一条消息**的可以使用 **$**，读完之后，**最新**就会删除。
...
> xread...$是读取这条命令之后接收到的最新消息，执行命令之前发布的消息将不会被包含在结果中。

STREAM类型消息队列的XREAD命令特点：

* 消息**可回溯**
* 一个消息可以被多个消费者读取（类似于 List 的实现思路）
* 可以阻塞读取
* 有**消息漏读**的风险
...
> 需要注意的是 **pending-list** 在消费者这边。

创建消费者组：
![1653577984924](.\Redis实战篇.assets\1653577984924.png)
key：队列名称
groupName：消费者组名称
**ID：起始ID标示，$代表队列中最后一个消息，0则代表队列中第一个消息**，这里表示要被消费者消费的信息。
...
">"：从下一个未消费的消息（这里的未消费消息位于 stream 中）开始 => **防止消息漏读**。
其它：根据指定id从（消费者的）pending-list中获取已消费但未确认的消息，例如0，是从pending-list中的第一个消息开始 => **消息确认机制，保证消息被至少消费一次**。
...
在探店笔记的详情页面，应该把给该笔记点赞的人显示出来，比如最早点赞的TOP5，形成**点赞排行榜**：
...
public Result queryBlogLikes(Long id) {
    ...
    List<UserDTO> userDTOS = userService.query()
            .in("id", ids).last("ORDER BY FIELD(id," + idStr + ")").list() // 直接 in 会导致顺序不正确，需要添加 ORDER BY。
            .stream()
...
关注是User之间的关系，是博主与粉丝的关系，**数据库中有一张tb_follow表来标示**（**多对多**表需要这样）：
...
以上两个功能和共同关注没有什么关系，大家可以自行将笔记中的代码拷贝到idea中就可以实现这两个功能了，我们的重点在于**共同关注功能**。
...
这一小节主要实现的是**向下滚动刷新**。如果有新的数据，下拉拉取新数据即可，不会影响滚动刷新的数据。
...
布隆过滤器的作用是什么？其可以用来告诉你 **“某样东西一定不存在或者可能存在”**。
